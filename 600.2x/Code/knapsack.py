...
def dToT(n, numDigits):
    """requires: n is a natural number less than 2**numDigits
      returns a ternary string of length numDigits representing the
              the decimal number n."""
...
def testYieldAllCombos():
    ...
    for combos in xrange(3**len(items)):
        combo=bags.next()
        for i in range(len(combo[0])):
            print combo[0][i].getName(),
        print
    #print "Bag 2: ",bag2
testYieldAllCombos()
...
#print combos.next().getName()

def TestdToT(n,NumDigits):
    for i in xrange(3**n):
        print dToT(i,NumDigits)
